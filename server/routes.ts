...
${pages.map(p => `  <url><loc>${baseUrl}${p}</loc><changefreq>daily</changefreq></url>`).join("\n")}
  ...
  // ── Chase Mode API ──────────────────────────────────────────
  app.post("/api/chase/check-proximity", async (req, res) => {
    try {
      const { latitude, longitude, radiusMiles, minScore, userId } = req.body;
      if (!latitude || !longitude) {
        return res.status(400).json({ message: "latitude and longitude required" });
      }
      const { checkProximity } = await import("./chase-mode");
      const results = await checkProximity(
        userId || "default",
        latitude,
        longitude,
        radiusMiles || 5,
      ...
      console.error("Chase proximity error:", error);
      res.status(500).json({ message: error.message });
    }
  });

  app.post("/api/chase/update-location", async (req, res) => {
    try {
      const { latitude, longitude, speedMph, heading, userId } = req.body;
      if (!latitude || !longitude) {
        return res.status(400).json({ message: "latitude and longitude required" });
      }
      const { updateLocation } = await import("./chase-mode");
      await updateLocation(userId || "default", latitude, longitude, speedMph, heading);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.post("/api/chase/register-device", async (req, res) => {
    try {
      const { pushToken, platform, userId } = req.body;
      if (!pushToken || !platform) {
        return res.status(400).json({ message: "pushToken and platform required" });
      }
      const { registerDevice } = await import("./chase-mode");
      await registerDevice(userId || "default", pushToken, platform);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.get("/api/chase/queue", async (req, res) => {
    try {
      ...
      const { getQueue } = await import("./chase-mode");
      const queue = await getQueue(userId, lat, lon);
      res.json(queue);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.post("/api/chase/action", async (req, res) => {
    try {
      const { leadId, action, latitude, longitude, notes, userId } = req.body;
      if (!leadId || !action) {
        return res.status(400).json({ message: "leadId and action required" });
      }
      const validActions = ["called", "knocked", "left_card", "scheduled", "not_interested", "completed", "skipped"];
      if (!validActions.includes(action)) {
        return res.status(400).json({ message: `Invalid action. Must be one of: ${validActions.join(", ")}` });
      }
      const { recordAction } = await import("./chase-mode");
      await recordAction(userId || "default", leadId, action, latitude, longitude, notes);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.get("/api/chase/lead/:id/summary", async (req, res) => {
    try {
      const { getLeadSummary } = await import("./chase-mode");
      const summary = await getLeadSummary(req.params.id);
      if (!summary) return res.status(404).json({ message: "Lead not found" });
      res.json(summary);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }

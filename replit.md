...
- **Multi-Market Architecture**: Config-driven support for market expansion without code changes. Active markets: DFW (4-county TX), Colorado Springs (El Paso County, CO), and Fort Collins (Larimer County, CO). All dashboard/ops/alert endpoints accept `?marketId=` for market-scoped data. Frontend pages pass `activeMarket.id` via `useMarket()` hook.
...
- **Fort Collins Open Data (Socrata)**: Building permits via `opendata.fcgov.com/resource/fvgz-viez.json`. Commercial permit types: General Alteration, New Com-Ind-Mixed-Use, New Secondary Building, Addition, Demolition. Contact names and phone numbers extracted from `b1_work_desc` field via regex (Job Contact/Jobsite Contact patterns). 384 total commercial permits, ~265 with extracted contacts.
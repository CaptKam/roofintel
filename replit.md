...
- **PropStream CSV/Excel Import**: Enrichment importer (`server/propstream-importer.ts`) that parses PropStream exports (CSV and Excel .xlsx/.xls), auto-detects 45+ column headers including skip trace fields, matches rows to existing leads by normalized address, and fills only null/sentinel fields. Supports property data (year_built, last sale date, lot size, school district, subdivision, land use, assessed value, building sqft) and skip trace data (mobile/landline phones, emails, owner person names behind LLCs, mailing addresses, Do Not Mail flags). Admin UI card with upload, progress tracking, and separate property/skip-trace enrichment breakdowns. Endpoints: `POST /api/import/propstream-csv`, `GET /api/import/propstream-csv/status`.
